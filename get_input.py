...
class Indata:
    ...
    def get_files(self):
        fname = ''.join(self.path + '/FINITE_VOLUME_CENTROID_COORDINATES.TXT')
        with open(fname, 'r') as f:
            self.centralCoordinats = np.array([float(line) for line in f])
        f.close()
        fname = ''.join(self.path + '/DOMAIN_SIZE.TXT')
        with open(fname, 'r') as f:
            self.domainSize = np.array([float(line) for line in f])
        f.close()
        ...
        self.numberOfGridPoints = round(np.size(self.moleFractions)/self.numberOfElements/self.numberOfPhases/np.size(self.timeList))
        self.path = 0